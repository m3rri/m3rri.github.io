...
function allPost() {
    ...
    if (allPost.length === 0) {
        ...
        return `export const allPost = ${JSON.stringify(sortListByDateOrTitle(allPost))};`;
    }
}
...
function getAllCategory() {
    ...
    return sortListByDateOrTitle(allPostMeta).map((meta) => {
        return {
...
function getMenuList() {
    ...
    const partList = allCategory
        .map((category) => category.part)
        .filter((part, index, array) => array.indexOf(part) === index);
    const nameList = partList.map((part) => {
        const nameList = allCategory
            .filter((category) => {
